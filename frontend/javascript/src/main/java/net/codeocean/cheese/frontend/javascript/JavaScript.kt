...
class JavaScript(
    ...
    init {
//        ScriptRuntime.addAdditionalPrefixes("kong")
    ...
    override fun run(workingDir: File) {
        val jvm =File(PathImpl.ASSETS_DIRECTORY,"jvm.apk")
        if (jvm.exists()){
            CoreFactory.createPlugins().install(jvm.absolutePath)
        }
        Hot.checkUpdate(1) {
    ...
    private fun execute(jsCode: String) {
        ...
        XLog.d("Rhino版本：" +  Context.getCurrentContext().getImplementationVersion());
