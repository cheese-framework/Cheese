...
//    - `considerAlpha`: 是否比较透明度通道
    ...
    private fun findMultiColors(
        ...
        XLog.e(list.joinToString { it.toString() })
        XLog.e(distance)
        val firstPoints =
            findColorInRange(
                img,
    ...
    override fun findMultiColors(
        ...
        if (!isMainProcess()) {
            ...
            for (item in (paths as Array<*>).toCollection(ArrayList())) {
                when (item) {
                    is List<*> -> {
                        val coordinates = item
                        if (coordinates.size == 3) {
                            val x = when (val coord = coordinates[0]) {
                                is Int -> coord                     // 处理 Integer
                                is Double -> coord.toInt()          // 处理 Double
                                is Number -> coord.toInt()          // 通用处理
                                else -> throw IllegalArgumentException("坐标必须是数字")
                            }

                            val y = when (val coord = coordinates[1]) {
                                is Int -> coord
                                is Double -> coord.toInt()
                                is Number -> coord.toInt()
                                else -> throw IllegalArgumentException("坐标必须是数字")
                            }
...
//                            val y = (coordinates[1] as Double).toInt()
                    ...
                    is Array<*> -> { // 新增对 Object[] 的支持
                        if (item.size == 3) {
                            val x = when (val coord = item[0]) {
                                is Int -> coord
                                is Long -> coord.toInt()
                                is Double -> coord.toInt()
                                is Float -> coord.toInt()
                                else -> 0  // 默认值或抛出异常
                            }

                            val y = when (val coord = item[1]) {
                                is Int -> coord
                                is Long -> coord.toInt()
                                is Double -> coord.toInt()
                                is Float -> coord.toInt()
                            ...
                            val color = item[2] as? String ?: "#000000"
                            pathList.add(intArrayOf(x , y , android.graphics.Color.parseColor(color)))
                        } else {
                            println("Unexpected array format: $item")
                        }
                    }
